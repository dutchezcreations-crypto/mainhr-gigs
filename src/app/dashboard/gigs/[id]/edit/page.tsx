...
export default function EditGigPage() {
      ...
      try {
        ...
        if (data) {
          setFormData({
            ...
            delivery_time: data.delivery_time || "3 days",
            revisions: data.revisions?.toString() || "2",
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Try uploading to Supabase storage bucket if a new image was selected
      if (imageFile) {
        try {
      ...
      const { error } = await supabase
        ...
        .update({
          ...
          images: [imageUrl] // Update images array with new cover image URL
        })
        .eq("id", id);
  ...
  return (
    <div className={styles.container}>
      <header className={styles.header}>
        ...
        <p className={styles.subtitle}>Update your service information and upload a stunning cover photo.</p>
      </header>
      ...
      <form onSubmit={handleSubmit}>
        <div className={styles.formCard}>
          ...
          <section className={styles.section}>
            <h2 className={styles.sectionTitle}><ImageIcon size={20} className="text-primary-600" /> Gig Cover Photo</h2>
            <input 
              type="file" 
              ref={fileInputRef} 
              style={{ display: "none" }} 
              ...
              {imagePreview ? (
                <div className={styles.previewContainer}>
                  ...
                  <div className={styles.previewOverlay}>
                    <ImageIcon size={32} />
                    <span className="font-bold text-sm mt-2">Change Cover Photo</span>
                  </div>
                </div>
              ) : (
                <>
